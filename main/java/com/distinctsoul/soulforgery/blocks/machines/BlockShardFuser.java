...
public class BlockShardFuser extends Block {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockInit.SHARD_FUSER);
	}
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if (active) {
			worldIn.setBlockState(pos, BlockInit.SHARD_FUSER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, true), 3);
		} else {
			worldIn.setBlockState(pos, BlockInit.SHARD_FUSER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, false), 3);
		}
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACING, EnumFacing.HORIZONTALS[(meta & 0b1100) >> 2]);
	}
